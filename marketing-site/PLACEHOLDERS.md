...
| `src/components/brand/{WunomoWordmark,WunomoMark,index}` + `public/brand/*.svg` + `src/app/{icon.svg,apple-icon.png,favicon.ico}` | Same paths under `dataops-agent/frontend/` | **Yes, verified byte-identical 2026-08-20** — `git hash-object` compared against the product's own git blobs directly (not eyeballed), every file an exact match. **This is the one place `design/BRAND_README.md` says the two repos must not diverge at all** — "copy this tree into BOTH the product repo and the marketing-site repo. Keep them byte-identical; when the logotype changes it changes in both or they drift." Unlike the tokens/components rows above (which are ports, adapted to this app's own needs), this tree is meant to be a mechanical copy, forever — if it ever needs a real edit, make it in the product first and re-copy here unchanged, not the other way round. `alpha-parallel.svg` (named in the README) doesn't exist in either repo yet — not an oversight here, the product doesn't have it either. |
