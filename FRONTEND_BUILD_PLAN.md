...
| 5 | **Non-password auth: Google OAuth + email-code** — **code-complete** | None | Real Google OAuth flow, callback endpoint, `email_verified`-gated account linking, built against real `GOOGLE_OAUTH_CLIENT_ID`/`SECRET` env vars (provided); **plus** the email-code method (Resend integration, `EmailLoginCode` table + rate limiting, auto-link on verify) — bundled into one phase since both share the `User` schema migration (nullable `hashed_password`, `google_id`, `email_verified`), the JWT `auth_method` handling, and the auto-link machinery designed together in Phase 3 | Email-code: **done** — fully live-verified, including a real, fully-automated Resend delivery round-trip (code sent, retrieved via Resend's own API, verified, JWT issued). Google: real calls made against Google's actual endpoints for everything short of the consent screen itself (URL generation, state validation, a real rejected-token-exchange call proving the code genuinely talks to Google); the full success path is regression-tested with Google's two calls mocked at the boundary. **The one remaining gate is a human clicking through Google's real consent screen — the agent cannot do this alone; requires the user, any real Google account.** Not "credentials pending" (they're provided) — "human-interaction pending," a different kind of gate than the Gemini-quota situation | Phase 3 |
