...
// Filtered copy of /sources (Wunomo UI-rebuild slice 6a, 2026-09-14) --
// same CRUD, same modal, the list is scoped to this project's resolved
// source set (lib/projectScope.ts) rather than the whole tenant. Sources
// is the root of Option B's join, not itself derived from anything else,
// so this is the one surface with no "manual/unscoped" exception to
// handle (see UI_REBUILD_INVENTORY.md §2's join table).
const SOURCE_TYPES = [
...
export default function ProjectSourcesTab() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      toast.push(
        `Source "${name}" created — not scoped to any agent yet, so it won't show here until you grant it from the Agents tab.`,
        "success"
      );
      resetModal();
      invalidate();
  ...
  const uploadMut = useMutation({
    ...
    onSuccess: (result) => {
      toast.push(
        `Uploaded "${result.filename}" — not scoped to any agent yet, so it won't show here until you grant it from the Agents tab.`,
        "success"
      );
      resetModal();
      invalidate();
  ...
  const list = (sources.data?.sources ?? []).filter((s) => sourceIds.has(s.id));
  const loading = sources.isLoading || scopeLoading;

  return (
    <div>
      <div className="flex items-center justify-between" style={{ marginBottom: 16 }}>
        <span className="text-muted text-sm">Sources this project's agents can reach.</span>
      ...
      {loading ? (
        ...
        <NoSourcesGrantedEmptyState projectId={projectId} surface="sources" />
      ) : list.length === 0 ? (
      ...
      >
        <div className="flex flex-col gap-3">
          <p className="text-muted text-xs">
            New sources aren&apos;t scoped to any agent — grant this one from the Agents tab afterward to
            see it in this list.
          </p>
          <Select
            id="source-type"
