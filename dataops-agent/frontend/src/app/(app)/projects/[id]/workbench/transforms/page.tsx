...
export default function ProjectTransformsTab() {
  ...
  const sources = (sourcesQuery.data?.sources ?? []).filter((s) => sourceIds.has(s.id));
  ...
  if (scopeLoading || sourcesQuery.isLoading) {
    return <Skeleton style={{ height: 300, borderRadius: 12 }} />;
  }
  if (sourceIds.size === 0) {
    return <NoSourcesGrantedEmptyState projectId={projectId} surface="transforms" />;
  }
            ...
            {runsQuery.isLoading ? (
              ...
              <div className="empty-state">
                ...
                <p className="text-muted text-sm" style={{ maxWidth: 380 }}>
                  Real SQL and Python transforms you run on this project&apos;s sources — or AXIOM runs on
                  your behalf in chat — appear here.
                </p>
              </div>
            ) : (
