...
# tenant_id/user_id/session_id are always force-injected server-side at
# execution time (agent_node's existing convention in dataops_agent.py) --
# never asked of the planning LLM, never accepted in a persisted or
# human-edited TaskStep.tool_args.
_SERVER_INJECTED_ARGS = {"tenant_id", "user_id", "session_id"}
...
async def generate_plan(tenant_id: str, user_id: str, goal: str, task_shape: TaskShape) -> list:
    ...
    for attempt in range(_MAX_GENERATION_ATTEMPTS):
        ...
        try:
            raw = await invoke_llm(
                ...
                tenant_id=tenant_id, user_id=user_id, request_type="task_planning",
            )
