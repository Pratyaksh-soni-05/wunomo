...
| 49 | Same bug as item 48, additional live confirmation on a third tenant. Found 2026-08-19 auditing Phase 0 of the frontend upgrade brief's demo-tenant question (`demo@axiom-yc.ai`, the tenant Phase 7's screenshots are meant to come from): `incidents` for this tenant had 111 rows at first check, 109 `OPEN` / 2 `RESOLVED` — all near-identical `"Stale data: Sales Orders (N.Nh overdue)"` / `"Stale data: Employee Records (N.Nh overdue)"` pairs, timestamped roughly every 15–60 minutes from 2026-08-16 08:51 through 2026-08-19 19:18 (still growing). Same root cause as item 48 (`_check_freshness()` in `services/tasks.py`, no dedup against an already-open incident for the same asset) — logged separately only because it hits the specific tenant the frontend brief's Phase 7 screenshots depend on: an Incidents-screen capture taken today would show duplicate rows, not the one clean incident `docs/SELF_TEST_GUIDE.md` §7.1 describes. **Re-checked 2026-08-19, same day, during Phase 4's dashboard audit: 123 `OPEN`** — up from 109, confirmed by direct query, not an estimate. That's +14 rows within a few hours of the *same day's* first check, on a tenant nobody is actively using — stronger, harder evidence for whoever fixes `_check_freshness()` than the original single snapshot: this isn't a slow trickle, it's happening continuously in the background right now. Not fixed — logged only, per instruction; rows deliberately left in place (deleting them to clean up a screenshot would hide the bug and let it keep regenerating). The Dashboard's "Open Incidents" KPI correctly shows the real, current count with no frontend-side filtering or dedup. Fix belongs with item 48's, not as new scope here. | — | Bug | none | Open |
