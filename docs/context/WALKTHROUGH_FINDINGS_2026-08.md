...
| 49 | Same bug as item 48, additional live confirmation on a third tenant. Found 2026-08-19 auditing Phase 0 of the frontend upgrade brief's demo-tenant question (`demo@axiom-yc.ai`, the tenant Phase 7's screenshots are meant to come from): `incidents` for this tenant has 111 rows, 109 `OPEN` / 2 `RESOLVED` — all 109 open rows are near-identical `"Stale data: Sales Orders (N.Nh overdue)"` / `"Stale data: Employee Records (N.Nh overdue)"` pairs, timestamped roughly every 15–60 minutes from 2026-08-16 08:51 through 2026-08-19 19:18 (still growing). Same root cause as item 48 (`_check_freshness()` in `services/tasks.py`, no dedup against an already-open incident for the same asset) — logged separately only because it hits the specific tenant the frontend brief's Phase 7 screenshots depend on: an Incidents-screen capture taken today would show 109 duplicate rows, not the one clean incident `docs/SELF_TEST_GUIDE.md` §7.1 describes. Not fixed — logged only, per instruction; rows deliberately left in place (deleting them to clean up a screenshot would hide the bug and let it keep regenerating). Fix belongs with item 48's, not as new scope here. | — | Bug | none | Open |
