...
| LLM layer | Groq (Llama 3.1) primary, Gemini path as alternate/fallback |
| Auth | JWT (python-jose), bcrypt, passlib |
| Testing | pytest + pytest-asyncio |
| Logging | structlog |
| Container runtime | Docker Compose |

**Local project path:** `C:\Personal Projects\workforce-agent`
**Services & ports:** Backend API `8000` · PostgreSQL `5432` · Redis `6379` · Celery worker (internal) · Celery beat (internal)

### 2.2 The 13 API segments (all under `/api/v1/`)
...
These four were debugged live and are easy to accidentally undo:

| # | File | Problem | Fix |
|---|---|---|---|
| 1 | `postgres_connector.py` | Endpoints each built their own DB connection → "no PostgreSQL user name specified" | Centralised all Postgres connections through one connector reading `connection_config` from the Source record |
| 2 | `services/llm_service.py` | `get_primary_llm()` always returned `ChatGoogleGenerativeAI`, so Groq/Llama models 404'd on the Gemini API | Route by model name — if it contains `llama`/`mixtral`/`gemma`, return `ChatGroq` |
| 3 | `api/v1/chat.py` (~line 63) | `datetime.now(timezone.utc)` is timezone-aware; the column is `TIMESTAMP WITHOUT TIME ZONE` → DataError on every chat save | Use `datetime.utcnow()` (timezone-naive) |
| 4 | Docker env loading | Editing `.env` and restarting didn't pick up new keys — old values stayed cached | Always `docker compose down && docker compose up -d`; verify with `docker compose exec backend printenv` |

**Two more recurring gotchas:**
- Source `host` must be `postgres` (the Docker service name), never `localhost` — containers resolve each other by service name.
- The Groq free tier's 12,000 TPM limit was hit by an oversized system prompt; the workaround was moving to `llama-3.1-8b-instant`.
...
docker compose logs -f backend        # or celery-worker / celery-beat
docker compose ps
docker compose exec backend pytest tests/test_api.py -v --tb=short
```
...
**Engineering**
- [ ] CI/CD "what is not yet built" gaps — see §12 of the CI/CD doc; close out remaining items.
- [ ] Frontend integration — the CI/CD doc specifies buttons, screens, and API mappings; the frontend itself is the next major build.
- [ ] SaaS readiness / production deployment — documented as a plan, not yet executed.
- [ ] Groq free-tier TPM ceiling is a live constraint; decide on paid tier or prompt-size budget before beta.
- [ ] Personalities 2+ (Analytics Agent is the natural second) — nothing built yet.
